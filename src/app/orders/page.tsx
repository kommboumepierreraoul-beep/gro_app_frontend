...
const payWithNotchPay = async () => {
  ...
  const handleInvoice = async (orderId: number) => {
    try {
      ...
      else toast.success(res.data.message || 'Facture générée');
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Impossible de générer la facture');
    }
  };
          ...
          <div className="lg:col-span-8 space-y-5">
                  ...
                  <div className="p-5">
                    ...
                    {order.status === 'draft' && cartItem && (
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        >
                          <span className="font-bold">{cartItem.quantity}</span> <span className="text-xs">— modifier</span>
                        </button>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-3" onClick={(e) => e.stopPropagation()}>
                      ...
                      {/* Facture PDF - visible pour tous les statuts après paiement (y compris completed) */}
                    ...
                    {order.address && <div className="mt-3 text-xs text-gray-500 flex items-center gap-1 pt-2 border-t border-gray-100">📍 {order.address}</div>}
                  </div>
                </div>
              );
            })}
            {filteredOrders.length === 0 && (
              <div className="bg-white rounded-xl p-12 text-center border border-gray-200">
                ...
                <Link href="/marketplace" className="px-6 py-2.5 bg-emerald-700 text-white text-sm rounded-full shadow-md hover:bg-emerald-800">Explorer le catalogue</Link>
              </div>
            )}
          </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
              ...
              <div className="relative pl-8">
                ...
                {dbOrders.slice(0, 3).map((o, idx) => (
                  <div key={o.uniqueKey} className={`${idx < 2 ? 'mb-6' : ''} relative`}>
                    ...
                    <p className="text-sm font-medium text-gray-800">{o.title}</p><p className="text-xs text-gray-500 mt-0.5">{o.date}</p>
                  </div>
                ))}
            ...
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 rounded-xl p-5 text-white shadow-md">
              <div className="flex items-center gap-4">
                ...
                <div><p className="text-xs uppercase text-white/70">Panier actuel</p><p className="text-base font-semibold">{items.length} article(s)</p><p className="text-sm text-white/80">{totalPrice.toLocaleString()} FCFA</p></div>
              </div>
            </div>
          </aside>
        </div>
      ...
      {cartModalProduct && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setCartModalProduct(null)}>
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold">Modifier la quantité</h2><button onClick={() => setCartModalProduct(null)}><XCircle size={22} className="text-gray-400"/></button></div>
            ...
            <div className="flex items-center justify-center gap-4 mb-6"><button onClick={() => setCartModalQty(q => Math.max(1, q-1))} className="w-10 h-10 rounded-full bg-gray-100"><Minus size={18}/></button><span className="text-2xl font-black w-12 text-center">{cartModalQty}</span><button onClick={() => setCartModalQty(q => q+1)} className="w-10 h-10 rounded-full bg-emerald-100"><Plus size={18}/></button></div>
            <div className="flex justify-between items-center mb-4 text-sm text-gray-500"><span>Total</span><span className="font-black text-gray-800 text-lg">{(cartModalProduct.price * cartModalQty).toLocaleString()} FCFA</span></div>
      ...
      {modalOpen && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center"><h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"><Package size={22} className="text-emerald-700" /> Détails</h2><button onClick={() => setModalOpen(false)}><XCircle size={24} className="text-gray-400"/></button></div>
            <div className="p-6 space-y-5">
              <div className="flex justify-between pb-3 border-b border-gray-100">
                <div><div className="text-sm text-gray-500">N° commande</div><div className="font-mono text-lg font-bold text-emerald-800">{selectedOrder.orderId}</div><div className="text-xs text-gray-400 mt-1">{selectedOrder.date || 'Non finalisée'}</div></div>
                <div><div className="text-sm text-gray-500">Statut</div><span className={`mt-1 inline-flex items-center gap-1.5 px-3 py-1 text-sm font-medium rounded-full ${statusConfig[selectedOrder.status]?.className || 'bg-gray-100'}`}>{statusConfig[selectedOrder.status]?.label || selectedOrder.status}</span></div>
              </div>
              <div className="bg-gray-50 rounded-xl p-3"><div className="font-medium">{selectedOrder.title}</div><div className="text-sm text-gray-500 mt-1">Prix : {selectedOrder.price.toLocaleString()} FCFA</div>{selectedOrder.address && <div className="text-sm text-gray-500 mt-1">📍 {selectedOrder.address}</div>}</div>
              <div className="flex justify-between items-center pt-3 border-t border-gray-100"><span className="text-lg font-bold">Total</span><span className="text-2xl font-extrabold text-emerald-800">{selectedOrder.price.toLocaleString()} FCFA</span></div>
