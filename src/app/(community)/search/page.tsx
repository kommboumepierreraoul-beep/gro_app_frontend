...
export default function SearchPage() {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      // ✅ Appel correct : searchPosts(query, page)
      // La page est optionnelle, par défaut 1
  ...
  // ── Chargement des pages suivantes (CORRIGÉ) ─────────────────────────────
  ...
  // ── Filtres ─────────────────────────────────────────────────────────────────
  // Note: Les filtres et le tri sont maintenant juste visuels
  // car l'API ne supporte pas ces paramètres
  const SortFilter = () => (
    <div className="flex items-center gap-2">
      <select
        ...
        onChange={(e) => {
          ...
          // Re-lancer la recherche avec le nouveau tri
          if (query.length >= 2) {
            performSearch(query);
          }
        }}
