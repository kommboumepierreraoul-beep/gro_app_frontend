    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="relative h-40 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
        {(profile as any).banner && (
          <Image src={(profile as any).banner} alt="Bannière" fill className="object-cover" />
        )}
      </div>
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-gray-400 pt-1">
            {(profile as any).location && (
              <span className="flex items-center gap-1">📍 {(profile as any).location}</span>
            )}
          ...
          {(profile as any).bio && (
            <p className="text-sm text-gray-600 mt-2 leading-relaxed">{(profile as any).bio}</p>
          )}
        </div>
